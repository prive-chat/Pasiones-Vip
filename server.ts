...
// Supabase Client for backend operations (lazily initialized)
...
function getSupabase(): ReturnType<typeof createClient> {
  if (!supabaseClient) {
    ...
    // Defensive check: if key is empty, use the fallback token to prevent crash
    const finalKey = supabaseServiceKey || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZxYnVwam51dmVmbHNobGhzbWp6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3MTQyNTM4NTgsImV4cCI6MjAzMDIyOTg1OH0.Yp9m_pE-p_p_p_p_p_p_p_p_p_p_p_p_p_p_p_p_p_p_p_p_p_p_p_p_p_p_p_p_p_p_p_p';
...
app.post("/api/send-push", async (req, res) => {
  ...
  try {
    ...
    const { data: subscriptionsData, error } = await getSupabase()
