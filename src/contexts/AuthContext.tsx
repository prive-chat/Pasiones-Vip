...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [isRecovery, setIsRecovery] = useState(() => {
    return window.location.search.includes('type=recovery') || window.location.hash.includes('type=recovery');
  });
  ...
  useEffect(() => {
    ...
    if (!supabaseUrl || !supabaseAnonKey || supabaseUrl.includes('placeholder')) {
      console.warn('Supabase credentials missing or invalid. Using fallback client configuration.');
    }
  ...
  return (
    <AuthContext.Provider value={{ user, profile, loading, isRecovery, setIsRecovery, refreshProfile, signOut }}>
      {children}
